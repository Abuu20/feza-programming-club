...
const LessonViewer = ({ 
  ...
  const getDisplayTitle = (attachment) => {
    // If admin provided a custom title, use that first
    if (attachment.title && attachment.title !== attachment.url) {
      // Check if title is not the auto-generated filename
      const isAutoGenerated = attachment.title.match(/^[a-f0-9-]+-\d+-[a-z0-9]+\.(png|jpg|jpeg|gif|webp|pdf)$/i);
      if (!isAutoGenerated) {
        return attachment.title;
      }
    }
    
    // If there's a description, use that as the title
    if (attachment.description) {
      return attachment.description;
    }
    
    // Otherwise, generate a friendly name based on file type
    ...
    // Try to extract a meaningful name from URL or generate one
    const urlParts = attachment.url.split('/').pop()?.split('-');
    if (urlParts && urlParts.length > 2) {
      // Remove timestamp and random hash to get original name idea
      const possibleName = urlParts.slice(2).join('-').replace(/\.(png|jpg|jpeg|gif|webp|pdf)$/i, '');
      if (possibleName && possibleName.length > 3) {
        return possibleName.charAt(0).toUpperCase() + possibleName.slice(1);
      }
    }
    
    return `${typeNames[fileType]} ${new Date(attachment.created_at || Date.now()).toLocaleDateString()}`;
  };
      ...
      return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden border hover:shadow-lg transition-all duration-300">
          ...
          <div className="p-3 bg-gray-50 text-sm border-t">
            ...
            {attachment.description && attachment.description !== displayTitle && (
              <p className="text-gray-600 mt-1 text-xs">{attachment.description}</p>
            )}
          </div>
        </div>
      );
