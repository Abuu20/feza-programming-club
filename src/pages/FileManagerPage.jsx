...
// ── Share Modal (students can share individually; only admin can broadcast) ───
const ShareModal = ({ file, onShare, onCancel, canBroadcast, currentUserId }) => {
  const [shareType, setShareType] = useState(canBroadcast ? 'all' : 'individual'); // 'all' | 'individual'
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShareType('individual')}
              ...
              <FaUser size={13} /> {canBroadcast ? 'Specific Students' : 'Share With'}
            </button>
          </div>
        </div>
...
const FileManagerPage = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-gray-50 overflow-hidden">
      ...
      {shareFile && (
        <ShareModal file={shareFile} onShare={shareFileToStudents} onCancel={() => setShareFile(null)}
          canBroadcast={isAdmin} currentUserId={user?.id} />
      )}
    </div>
  );
};
