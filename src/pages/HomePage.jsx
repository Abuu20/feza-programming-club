...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      {/* Latest Activities Section - Using uploaded images */}
      ...
      <section className="py-20">
        <div className="container-custom">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-4xl font-bold text-primary-500 mb-4">Announcements</h2>
              <p className="text-xl text-gray-600">Stay updated with club news</p>
            </div>
            <button
              ...
              {announcements.length > 0 ? (
                announcements.map((announcement) => (
                  <div
                    key={announcement.id}
                    onClick={() => navigate('/announcements')}
                    className="group bg-white rounded-2xl shadow-lg p-8 cursor-pointer hover:shadow-xl transition border border-gray-100"
                  >
                    <div className="bg-secondary-50 w-12 h-12 rounded-xl flex items-center justify-center mb-4">
                      <BellIcon className="w-6 h-6 text-secondary-500" />
                    </div>
                    <h3 className="text-xl font-bold text-primary-500 mb-3">{announcement.title}</h3>
                    <p className="text-gray-600 mb-4 line-clamp-3">{announcement.content}</p>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">{formatDate(announcement.created_at)}</span>
                      <span className="text-secondary-500 group-hover:translate-x-2 transition">→</span>
                    </div>
                  </div>
                ))
              ) : (
